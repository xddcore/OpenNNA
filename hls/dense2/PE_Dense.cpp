...
#pragma HLS INTERFACE m_axi bundle=INPUT_Neure_weights depth=9216 port=PE_Neure_weights offset=slave
#pragma HLS INTERFACE m_axi bundle=INPUT_Neure_bias depth=1024 port=PE_Neure_bias offset=slave
...
#pragma HLS INTERFACE s_axilite bundle=CTRL_BUS port=return
	...
	for (int weights_bias_number = 0; weights_bias_number < 2; weights_bias_number++)//neure_number PE_Register[4]
	{
		/*+bias*/
		...
		//在这一步顺便将多唯索引压成一维来访问
		for (int input_fmap_size = 0; input_fmap_size < 18; input_fmap_size++)//对于全连接网络来说,它是一维的,所以input_fmap_size=input_fmap_size*input_fmap_size*input_fmap_channel
		{
			/*+bias*/
			/*非常重要:清空计算中间量 下面0,0,0位置等于权重,对于多次计算来说,清空了上一次的计算结果,避免影响本次计算*/
